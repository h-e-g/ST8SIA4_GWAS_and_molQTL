#!/usr/bin/env python
# script obatained from https://github.com/guigolab/ggsashimi
...
def setup_R_script(h, w, b, label_dict):
        s = """
        .libPaths("/pasteur/helix/projects/evo_immuno_pop/single_cell/resources/R_libs/4.1.0")
        library(ggplot2)
        library(grid)
        library(gridExtra)
        library(data.table)
...
def make_R_lists(id_list, d, overlay_dict, aggr, intersected_introns, out_prefix):
        s = ""
        aggr_f = {
                "mean": mean,
                "median": median,
                ...
                if not overlay_dict:
                        ...
                        if aggr and "_j" not in aggr:
                                ...
                                if intersected_introns:
                                        x, y = shrink_density(x, y, intersected_introns)
                        #dons, accs, yd, ya, counts = [], [], [], [], []
                # Prepare string data for density_list file
                density_content = "\n".join(f"{x_val}\t{y_val}" for x_val, y_val in zip(x, y))
                # Filename using the current id
                density_filename = f"{out_prefix}_density_list_{k}.txt"
                with open(density_filename, 'w') as density_file:
                  density_file.write("x\ty\n")  # Optional: Write header
                  density_file.write(density_content)
                # Prepare string data for junction_list file
                junction_content = "\n".join(
                          f"{don}\t{acc}\t{yd_val}\t{ya_val}\t{count}" 
                           for don, acc, yd_val, ya_val, count in zip(dons, accs, yd, ya, counts)
                        )
                junction_filename = f"{out_prefix}_junction_list_{k}.txt"
                # Write junction content to a file
                with open(junction_filename, 'w') as junction_file:
                  junction_file.write("x\txend\ty\tyend\tcount\n")  # Optional: Write header
                  junction_file.write(junction_content)
...
#       args.bam = "/nfs/no_backup/rg/epalumbo/projects/tg/work/8b/8b0ac8705f37fd772a06ab7db89f6b/2A_m4_n10_toGenome.bam"
        ...
        for strand in bam_dict:
                ...
                R_script += make_R_lists(id_list, bam_dict[strand], overlay_dict, args.aggr, intersected_introns, out_prefix)
